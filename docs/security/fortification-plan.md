...
| F23 | Canal de eventos de seguridad + kill-switch | 🟡 2026-08-21 | **Costura construida con la regla hecha tipo**: `SecurityEventKind` es un conjunto **cerrado y opaco**, sin campo para mensaje, usuario ni registro — un canal que acepta texto libre es el que acaba llevando «unlock failed for Dr. Pérez, paciente 11111111-1» a un log de servidor. Falla en silencio por doctrina. El stand-in se llama `NoOpSecurityEventReporter` **a propósito**: LumeMed embarcó ese canal cableado a un no-op y la plataforma nunca recibió un evento (tablero §3). ADR-0023. **Mitad cliente CONSTRUIDA 2026-08-25** (ADR-0026, bitácora 0024): `HttpSecurityEventReporter` sobre el stack endurecido —no alrededor—, con 7 tests contra un **doble del contrato** (servidor guionado detrás del stack real). Propiedades probadas: nunca lanza (500, red muerta, todo kind), POST **sin reintento** (T13: sin idempotencia, un reintento convierte un evento en tres), sólo cruza el kind opaco, y el log del stack no lo contiene. **Encontró un defecto en el stack embarcado**: la cancelación llega envuelta, la rama por tipo no la ve, y un llamador cancelado recibe un `AppError` — caracterizado con test y cerrado con `check-cancellation-guard.sh`. **Falta el cableado** (no hay base URL ni auth) y **el esquema del cuerpo, que es de la plataforma**: `backend-requests/0004`. |
